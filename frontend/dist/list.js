...
window.onload = function () {
  ...
  var updateMember = function updateMember(member) {
    ...
    view: function view(vnode) {
      ...
      events.forEach(function (event) {
        ...
        if (!already_left && event.event_type == 'Club' && event.division == 'Club' && event.class == 'Promotion') {
          badges.push({
            type: 'success',
            text: 'Joined on ' + event.date
          });
        }

        if ((event.event_type.includes('Kyu') || event.event_type.includes('Dan')) && event.class == 'Promotion') {
          var grade = event.event_type.substring(event.event_type.length - 1) + '. ' + event.event_type.substring(0, event.event_type.length - 1);
      ...
      if (current_grade['Judo']) {
        badges.push({
          type: 'success',
          text: 'Promoted to ' + current_grade['Judo'].grade + ' on ' + current_grade['Judo'].date
        });
      }
      ...
      if (current_grade['Jujitsu']) {
        badges.push({
          type: 'success',
          text: 'Promoted to ' + current_grade['Jujitsu'].grade + ' on ' + current_grade['Jujitsu'].date
        });
      }

      return [m('', badges.map(function (badge) {
        return m('span.badge.badge-pill.badge-' + badge.type, badge.text);
      })), m('table.table.table-hover.col-12', [m('thead', m('tr', [m('th', 'Event'), m('th', 'Division'), m('th', 'Class'), m('th', 'Date'), m('th', 'Comment')])), m('tbody', [[m('tr', [m('td[colspan=4]', 'Add new event')])], events.slice(0).reverse().map(function (event) {
        return [m('tr', [m('td', event.event_type), m('td', event.division), m('td', event.class), m('td', event.date), m('td', event.comment)])];
      })])])];
    }
  };
  var MembersList = {
